...
#'
#' @param pkgs <chr> package name(s)
#'
#' @return data set names per package
#' @export
get_all_ds_per_pkg <- function(
  ...
  for (p in unique_pkg_names) {
    ...
    all_ds_tbl <- dplyr::bind_rows(all_ds_tbl, pkg_tbl)
  }
  return(all_ds_tbl)
}
...
#'
#' "pkgs" notes
#' 1: named packages are assumed to be installed
#' 2: length must be consistent with "datasets"
#'
#' @param pkgs <chr> package name(s)
#' @param datasets <chr> names of data sets per package
#'
#' @return selected data set names per package
#' @export
